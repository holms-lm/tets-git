...
class ControllerCheckoutGuest extends Controller {
	...
	public function save() {
		...
		if (!$json) {
		    //отключил обязательное поле имя
			if ((utf8_strlen(trim($this->request->post['firstname'])) < 1) || (utf8_strlen(trim($this->request->post['firstname'])) > 32)) {
				$json['error']['firstname'] = $this->language->get('error_firstname');
			}

            //отключил обязательное поле Фамилия
//            if ((utf8_strlen(trim($this->request->post['lastname'])) < 1) || (utf8_strlen(trim($this->request->post['lastname'])) > 32)) {
//				$json['error']['lastname'] = $this->language->get('error_lastname');
//			}

            //отключил обязательное поле потча
            if ((utf8_strlen($this->request->post['email']) > 96) || !filter_var($this->request->post['email'], FILTER_VALIDATE_EMAIL)) {
				$json['error']['email'] = $this->language->get('error_email');
			}

            //отключил обязательное поле телефон
			if ((utf8_strlen($this->request->post['telephone']) < 3) || (utf8_strlen($this->request->post['telephone']) > 32)) {

				$json['error']['telephone'] = $this->language->get('error_telephone');
			}

            //отключил обязательное поле адресс
//            if ((utf8_strlen(trim($this->request->post['address_1'])) < 3) || (utf8_strlen(trim($this->request->post['address_1'])) > 128)) {
//				$json['error']['address_1'] = $this->language->get('error_address_1');
//			}

            //отключил обязательное поле город
...
//			}
            ...
            //отключил обязательное поле индекс
//            if ($country_info && $country_info['postcode_required'] && (utf8_strlen(trim($this->request->post['postcode'])) < 2 || utf8_strlen(trim($this->request->post['postcode'])) > 10)) {
//				$json['error']['postcode'] = $this->language->get('error_postcode');
//			}

            //отключил обязательное поле страна
//            if ($this->request->post['country_id'] == '') {
//				$json['error']['country'] = $this->language->get('error_country');
//			}

            //отключил обязательное поле регион
